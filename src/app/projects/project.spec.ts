...
describe('Eqips', () => {
  describe('`selectedExpense` store', () => {
    it('returns null by default', () => {
      let defaultState = selectedProject(undefined, {type: 'random', payload: {}});

      expect(defaultState).toBeNull();
    });

    it('`SELECT_EXPENSE` returns the provided payload', () => {
      let selectProject = selectedProject(undefined, {type: 'SELECT_PROJECT', payload: 'payload'});

      expect(selectedProject).toBe('payload');
    });
  });
